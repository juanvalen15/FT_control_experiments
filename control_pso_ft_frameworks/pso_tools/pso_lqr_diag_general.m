function [Q,R,best,GB,iters,time]=pso_lqr_diag_general(Ac,Bc,h,delay,m,Cp,Cg,w,iter,Sett,max_x,max_u,Sat_u)
...
% Sat_u=maximum and minumum value of the controller output
%
% Robinson Medina 
...
for cont2=1:m 
    ...
    %Q=([1/max_x^2*ones(n,1); old_u]).*rand(states,1);
...
for cont2=1:iter
    ...
    for cont=1:m        
        ...
        try
            ...
            %St= setim4O(Ac,Bc,K,F,pipes,10*pi/180,delay,h,Sett,Sat_u,0); % computes the settling time
...
%         if St<2  % robinson: this is used for the pato system for simple
%         control experiments less than 2 seconds setling time
%             break
%         end
        % update global best
        if fitness>X_gb{1}
...
fprintf('finished: global best=%d, \n', -X_gb{1});
% Q=diag(diagonal(1:states)); % Q matrix LQR
% R=diag(diagonal(states+1:states+inp));
time=toc;
iters=cont2;
best=X_gb{2};
GB=-X_gb{1};