...
#define DEBUG_VARIABLES 18 // variables that will be saved to external memory
  ...
  if ( location_counter == 1 ){
    sensing_instant = 1;
      ...
      if(i<N-1){
        ...
        augmented_state[i][0] = sensed_data[i][0];
      }
      else{
        augmented_state[i][0] = old_actuation_signal[i][0];
      }
  	}
  }
  else{
    sensing_instant = 0;
    for(i=0; i<N; i++){
        augmented_state[i][0] = old_augmented_state[i][0];
  	}
  }
...
int main ( void )
{
  ...
  for(k_loop=0; k_loop < LOOPS; k_loop++){
	...
	xil_printf("k, t21, t31, ref, m, F, K0, s0, s1, s2, s3, s4, sI, uA, uB, u, aS, aI\n");
	...
	for(k_loop=0; k_loop < LOOPS; k_loop++){
		...
		xil_printf("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", k_temp, t21_lsb, t31_lsb, ref_temp, m_temp, F_temp, K0_temp, s0_temp, s1_temp, s2_temp, s3_temp, s4_temp, sI_temp, uA_temp, uB_temp, u_temp, aS_temp, aI_temp);
