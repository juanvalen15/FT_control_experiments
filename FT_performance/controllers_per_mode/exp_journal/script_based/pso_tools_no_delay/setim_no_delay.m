function St= setim_no_delay(Ac,Bc,Phi,Gamma,K,F,R,delay,Ts,Sett)%,Sat_u)
...
assert(isa(Ac,'double'));assert(all(size(Ac)<=[6 6]));assert(isreal (Ac));
assert(isa(Bc,'double'));assert(all(size(Bc)<=[6 1]));assert(isreal (Bc));
assert(isa(Phi,'double'));assert(all(size(Phi)<=[6 6]));assert(isreal (Phi));
assert(isa(Gamma,'double'));assert(all(size(Gamma)<=[6 1]));assert(isreal (Gamma));
...
[n,~]=size(Bc);
%states=1+n; % with delay
states=n; % no delay
h=Ts;
%[Phi,Gamma] = Discretize_no_delay(Ac,Bc, h);
