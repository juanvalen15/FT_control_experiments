...
max_x = [1e-6; 1e-6; 100; 100];   % maximum deviation in all the states x, the smaller the better : FOR NOMINAL SAMPLING PERIOD
max_u = 1.5;   % maximum controller output

states = 1 + n;                                 % system states
% m      = round(2*states*5 + exp(0.8*states) ); % population size
m      = 200;
...
for cont = 1:loops
    ...
    [Q, R, X_pb, GB, iters, time] = pso_lqr_diag_general(Ac_plant, Bc_plant, Ts, application_delay, ...
