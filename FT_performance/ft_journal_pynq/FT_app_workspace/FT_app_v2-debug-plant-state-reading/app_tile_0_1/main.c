...
#define LOOPS 800 // control loops
#define DEBUG_VARIABLES 19 // variables that will be saved to external memory
...
int main ( void )
{
  ...
  // for(int k=0; k < ws_reference_len; k++){
    ...
    // OLD VARIABLES UPDATE ----------------------------------------------------
		...
		*(shared01_ext_uint32 + (k_loop +1*LOOPS)) = (uint32_t)tdiff21;
		*(shared01_ext_uint32 + (k_loop +2*LOOPS)) = (uint32_t)tdiff31;

		*(shared01_ext_int + (k_loop +3*LOOPS)) = (int)(reference[0][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +4*LOOPS)) = test_m;

		*(shared01_ext_int + (k_loop +5*LOOPS)) = (int)(F[0][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +6*LOOPS)) = (int)(K[0][0]*FLOAT_TO_INT);

		*(shared01_ext_int + (k_loop +7*LOOPS)) = (int)(augmented_state[0][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +8*LOOPS)) = (int)(augmented_state[1][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +9*LOOPS)) = (int)(augmented_state[2][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +10*LOOPS)) = (int)(augmented_state[3][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +11*LOOPS)) = (int)(augmented_state[4][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +12*LOOPS)) = sensingI;

		*(shared01_ext_int + (k_loop +13*LOOPS)) = (int)(new_control_input[0][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +14*LOOPS)) = (int)(test_actuation[0][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +15*LOOPS)) = actuationI;

		*(shared01_ext_int + (k_loop +16*LOOPS)) = (int)(new_control_inputA[0][0]*FLOAT_TO_INT);
		*(shared01_ext_int + (k_loop +17*LOOPS)) = (int)(new_control_inputB[0][0]*FLOAT_TO_INT);

		*(shared01_ext_int + (k_loop +18*LOOPS)) = ws_errors[k_loop];
	...
	int s0_temp, s1_temp, s2_temp, s3_temp, s4_temp = 0;
	int sI_temp = 0;

	int uA_temp, uB_temp, u_temp = 0;
	int aS_temp = 0;
	int aI_temp = 0;

	int e_temp = 0;

	xil_printf("k, e, t21, t31, ref, m, F, K0, s0, s1, s2, s3, s4, sI, uA, uB, u, aS, aI\n");

	for(k_loop=0; k_loop < LOOPS; k_loop++){

		k_temp = *(shared01_ext_int + (k_loop +0*LOOPS));

		t21_lsb = *(shared01_ext_uint32 + (k_loop +1*LOOPS));
		t31_lsb = *(shared01_ext_uint32 + (k_loop +2*LOOPS));

		ref_temp = *(shared01_ext_int + (k_loop +3*LOOPS));
		m_temp = *(shared01_ext_int + (k_loop +4*LOOPS));

		F_temp = *(shared01_ext_int + (k_loop +5*LOOPS));
		K0_temp = *(shared01_ext_int + (k_loop +6*LOOPS));

		s0_temp = *(shared01_ext_int + (k_loop +7*LOOPS));
		s1_temp = *(shared01_ext_int + (k_loop +8*LOOPS));
		s2_temp = *(shared01_ext_int + (k_loop +9*LOOPS));
		s3_temp = *(shared01_ext_int + (k_loop +10*LOOPS));
		s4_temp = *(shared01_ext_int + (k_loop +11*LOOPS));
		sI_temp = *(shared01_ext_int + (k_loop +12*LOOPS));

		u_temp = *(shared01_ext_int + (k_loop +13*LOOPS));
		aS_temp = *(shared01_ext_int + (k_loop +14*LOOPS));
		aI_temp = *(shared01_ext_int + (k_loop +15*LOOPS));

		uA_temp = *(shared01_ext_int + (k_loop +16*LOOPS));
		uB_temp = *(shared01_ext_int + (k_loop +17*LOOPS));

		e_temp = *(shared01_ext_int + (k_loop +18*LOOPS));

		xil_printf("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", k_temp, e_temp, t21_lsb, t31_lsb, ref_temp, m_temp, F_temp, K0_temp, s0_temp, s1_temp, s2_temp, s3_temp, s4_temp, sI_temp, uA_temp, uB_temp, u_temp, aS_temp, aI_temp);
